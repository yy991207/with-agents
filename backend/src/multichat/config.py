...
class Settings(BaseSettings):
    ...
    def _validate_judge_in_agents(self) -> "Settings":
        """yaml 种子合法性校验 judge.agent 必须出现在 agents 段中

        只校验 yaml 阶段 运行时 judge 指针由 DB settings 集合管理 不走这条路
        """
...
def load_settings(path: str | Path | None = None) -> Settings:
    ...
    _logger.info(
        ...
        llm_key_tail=_mask_key(settings.key),
        agents=sorted(settings.agents.keys()),
        judge_agent=settings.judge.agent,
    )
    return settings
