"""deepagents 实例构建器 每个agent一个reply实例 支持热替换与按用户懒构建

预创建策略:
    每个 agent 构造一个 deep_agent reply 实例
    - {agent_name}: 完整能力 鼓励规划+工具调用 流式回复
    任意数量 agent 都允许 启动时 initialize 一次 之后改 agent 配置走 reload
    新增 agent 也走 reload 追加 删除 agent 赑 unregister 移除

MCP 工具注入(2026-05-23):
    构建时会从数据库 mcp_config 文档读取 MCP 服务器配置
...
async def _build_one(
    ...
    *,
    ...
    # 注入共享 tool(current_time/http_get) + MCP 工具
...
class DeepAgentRegistry:
    ...
    def unregister(self, name: str) -> None:
        """删除 agent 时同步移除其实例及所有按用户缓存"""
        # 移除系统级实例
        self._instances.pop(name, None)
        # 移除所有按用户缓存 (key 格式 {user_id}:{name})
        to_remove = [k for k in self._instances if k.endswith(f":{name}")]
        for k in to_remove:
            self._instances.pop(k, None)
            self._versions.pop(k, None)
        _logger.info("deep_agents 注销", name=name)

    def get(self, agent_name: str) -> CompiledStateGraph:
        """读端 不加锁 dict.get 在 CPython GIL 下是原子的 仅用于系统级实例"""
        inst = self._instances.get(agent_name)
        if inst is None:
            raise KeyError(f"deep_agent not found: {agent_name}")
        return inst

    async def get_or_build(
        self, agent_name: str, *, owner_user_id: str | None = None
    ) -> CompiledStateGraph:
        """按用户获取或懒构建 agent 实例

        - owner_user_id 为 None: 返回系统级实例 (同 get)
        - 有值: 缓存 key 为 {user_id}:{name}
          版本过期则重新构建 MCP/Skills 按用户加载
        """
        if owner_user_id is None:
            return self.get(agent_name)

        key = f"{owner_user_id}:{agent_name}"
